...
class Password_Change(customtkinter.CTk):
    def __init__(self, root):
        ...
        self.root.title("Cab Booking System | Change Customer Password")
        ...
        currentpass_lbl=customtkinter.CTkLabel(master=self.root, text="New Password: ", font=font720, bg_color="#2b2b2b")
        currentpass_lbl.place(x=60, y=150)

        currentpass_txt=customtkinter.CTkEntry(master=self.root, font=font720,show='*', width=200)
        currentpass_txt.place(x=240, y=150)
        ...
        confirmpass_txt = customtkinter.CTkEntry(master=self.root, show='*', font=font720, width=200)
        confirmpass_txt.place(x=240, y=220)



        def show_password():
            if i.get() == 1:
                confirmpass_txt.configure(show='')
                currentpass_txt.configure(show='')
            else:
                confirmpass_txt.configure(show='*')
                currentpass_txt.configure(show='*')
        ...
        def change_password():
            ...
            password1=currentpass_txt.get()
            newpassword=confirmpass_txt.get()
